...
print('The accuracy on the initial_x set is {:.2f}'.format(test_accuracy)) #0.85

advs2 = advs
# test the accuracy of adversarial samples
activation = forward_step(advs2, layers, n_each, False)
y_pred = np.argmax(activation, axis=1)
adv_accuracy = metrics.accuracy_score(y_trues, y_pred)
print('The accuracy on the adversarial samples is {:.2f}'.format(adv_accuracy)) #0.58
